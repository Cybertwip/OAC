...
StackedAnimation::StackedAnimation(std::shared_ptr<Animation> animation, int startTime, int endTime)
: _animation(animation), _startTime(startTime), _endTime(endTime), _duration(animation->get_duration()){
	
}

void AnimationComponent::init_animation()
{
	custom_ticks_per_second_ = 60.0f;
	// current_time_ = 0.0f;
	// is_stop_ = false;
